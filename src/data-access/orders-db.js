module.exports = function makeOrdersDb({ makeQuery }) {
    ...
    async function insert({ order }) {
        return await makeQuery({ sql: "INSERT INTO orders (user_id, products_ids, date, price, card, payment_made) VALUES (?,?,?,?,?,?)",
            params: [order.userId, order.productsIds, order.date, order.price, order.card, order.paymentMade]
        });
    }
